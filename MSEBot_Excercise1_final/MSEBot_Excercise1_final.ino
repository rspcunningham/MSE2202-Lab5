// the weaker motor should be 1, the stronger motor a bit less than 1
...
void loop() {
  ...
  if (btRun) {
    ...
    switch (runningState) {
      ...
      case 1: //drive forwards and watch for beacon change
        runMotors(0, 1);
      ...
      case 2: //2000 ms hold on beacon hit
        runMotors(4, 0);
        ...
        if (millis() >= times[2] + 2000) runningState = 3;
        times[3] = millis();
      ...
      case 3: //reverse when beacon stop code recieved
        runMotors(1, 1);
        if (ENC_vi32LeftOdometer == leftEncTarget || ENC_vi32RightOdometer == rightEncTarget) runningState = 4;
        times[4] = millis();
      ...
      case 4: //2000 ms hold after reaching half distance
        runMotors(4, 0);
        if (millis() >= times[4] + 2000) {
          runningState = 5;
          ENC_ClearLeftOdometer(); //Re-zero encoder distances
      ...
      case 5: //turn 180º
        ...
        if (ENC_vi32LeftOdometer < -40) runningState = 6;
        times[6] = millis();
        break;
